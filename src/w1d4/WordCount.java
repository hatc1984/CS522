...
public class WordCount {
    ...
    public WordCount(int numberOfMapper, int numberOfReducer) throws IOException {
        ...
        // initialize mappers and reducer objects, then read files
        initialization();
    ...
    private void initialization() throws IOException {
        ...
        mappers = new InMapperWordCount[this.numberOfMapper];
        reducers = new Reducer[this.numberOfReducer];


        // initialize all the objects
        for(int i = 0; i < mappers.length; i++) {
            mappers[i] = new InMapperWordCount("src/w1d4/datatest/input" + i + ".txt");
    ...
    private void map() throws IOException {
        for(int i = 0; i < mappers.length; i++) {
            ...
            mappers[i].initialize();

            for(String term : mappers[i].wordList()) {
                mappers[i].map(term);
            }

            mappers[i].close();

            mapperOutputs.get(i).addAll(mappers[i].get());
        }
    }
    ...
    public void showMapperOutput() {
        ...
        for(int i = 0; i < mapperOutputs.size(); i++) {
            System.out.println("InMapperWordCount " + i + " Output: ");
            for(PairW<String, Integer> p : mapperOutputs.get(i)) {
    ...
    public void showPartitionOutput() {
        ...
        for(int i = 0; i < partitions.size(); i++) {
            for(int j = 0; j < partitions.get(i).size(); j++) {
                System.out.println("Pairs send from InMapperWordCount " + i + " Reducer " + j);
